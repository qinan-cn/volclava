...
#define FREE_MEMORY \
    { \
        /* req.names[i] point to caller strings; only free the array */ \
        FREEUP(req.names); \
        /* req.queue/user/project/hosts point to caller strings or */ \
        /* string literals, must NOT be freed. */ \
    }
...
#define FREE_REPLY_BUFFER \
    ...
    if (numNames > 0) {
        ...
        for (i = 0; i < numNames; i++) {
            if (names[i] && strlen(names[i]) + 1 < MAXHOSTNAMELEN) {
                req.names[i] = names[i];
                cc += MAXHOSTNAMELEN;
            } else {
                FREE_MEMORY;
