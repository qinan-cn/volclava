...
static char * getLimitKey4Job(RL_LIMIT_T *limit, struct jData *jp, char *hostname);
...
static int *getRLBitmap(const char *value, int consumerPos);
static int *getRLBitmap4HLimit(const char *value);
static int *getRLBitmap4Query(const char *value, int consumerPos, int *isFree);
static int **getRLBitmap4Job(struct jData *jp, char *hostname);
static char *getLimitKey(RL_LIMIT_T *limit, char *hostname,
 ...
 * memValueToMB
 * Description:
 *     Convert a MEM/SWP/TMP limit value from the unit configured via
 *     LSF_UNIT_FOR_LIMITS (lsf.conf) to MB. Uses 1024-based conversion
 *     (binary), matching the pattern in mbd.init.c for queue/host rlimits.
 *     This keeps limit values in the same unit (MB) as jobResVal->val[MEM]
 *     and hp->maxMem for consistent comparisons.
 *
 * Input:
 *     value [in]: configured value in LSF_UNIT_FOR_LIMITS unit
 *
 * Return:
 *     value in MB
 ********************************************************************************/
static float
memValueToMB(float value)
{
    int j;
 ...
 * memValueFromMB
 * Description:
 *     Reverse of memValueToMB: convert an MB value back to the unit configured
 *     via LSF_UNIT_FOR_LIMITS for display purposes.
 ********************************************************************************/
static float
memValueFromMB(float valueMB)
{
    int j;
    for (j = 0; j < unitForLimits; j++) {
        valueMB /= 1024;
...
freeRLJobLimitsCache(RL_JOB_LIMITS_CACHE_T *cache)
{
    ...
    if (cache->rlBitmaps != NULL) {
        FREEUP(cache->rlBitmaps);
    }
        ...
        *end = '\0';
        ...
        if (!isPercent
            && ((*resources)[nRes].type == RL_RESOURCE_TYPE_MEM
                || (*resources)[nRes].type == RL_RESOURCE_TYPE_SWP
                || (*resources)[nRes].type == RL_RESOURCE_TYPE_TMP)) {
            (*resources)[nRes].value = memValueToMB(fval);
        }
                  ...
                  int type)
{
    ...
    if (!isPercent
        ...
        (*resources)[*nResources].value = memValueToMB((*resources)[*nResources].value);
    }
...
do_Limit(struct lsConf *conf, char *fname, int *lineNum)
{
    ...
    if (!isValidName(keyList[RL_KEY_NAME].val)) {
        ls_syslog(LOG_WARNING, _i18n_msg_get(ls_catd, NL_SETN, 20,
                "%s: %s(%d): Invalid NAME <%s>; must contain only letters, digits, underscore or hyphen, skipping this limit"),
 ...
 *     USER/PROJECT). Bit i=1 means limit i matches the given value in this
 *     consumer dimension. Uses rlBitmapCache for caching with key format
 *     "value@pos".
 *
 * Input:
 *     value [in]: The consumer value to match (e.g. hostname, queue name).
 *                 May be NULL.
 *     pos [in]: Consumer position (RL_CONSUMER_POSITION_HOST/QUEUE/USER/PROJECT)
 *
 * Return:
 *     int *: Bitmap owned by rlBitmapCache, do NOT free. NULL on error.
 ********************************************************************************/
static int *
getRLBitmap(const char *value, int consumerPos)
{
 ...
 * getRLBitmap4HLimit
 * Description:
 *     Generate a HOST-dimension bitmap for candidate host filtering. Based on
 *     getRLBitmap() for HOST consumer, but clears bits for limits whose host
 *     consumer is NULL/empty/"all" and in SHARED mode, since such limits do
 *     not differentiate among hosts and should not constrain candidate host
 *     selection.
 *
 *     Uses rlBitmapCache with key format "value@pos@sch".
 *
 * Input:
 *     value [in]: Host name to match. May be NULL.
 *
 * Return:
 *     int *: Bitmap owned by rlBitmapCache, do NOT free. NULL on error.
 ********************************************************************************/
static int *
getRLBitmap4HLimit(const char *value)
{
    static char fname[] = "getRLBitmap4HLimit";
    int *srcBitmap;
    int *cachedBitmap;
    int i;
    char cacheKey[MAXLINELEN];
    ...
    /* Build cache key: value@pos@sch */
    snprintf(cacheKey, sizeof(cacheKey), "%s@%d@sch",
             (value != NULL) ? value : "null", RL_CONSUMER_POSITION_HOST);
    ...
    /* Generate base bitmap via getRLBitmap() */
    srcBitmap = getRLBitmap(value, RL_CONSUMER_POSITION_HOST);
    if (srcBitmap == NULL) {
        return NULL;
    }

    /* Copy and clear bits for SHARED-mode host consumers with
     * NULL/empty/"all" value (non-discriminating limits). */
    cachedBitmap = (int *)my_calloc(rlBitmapSize, sizeof(int), fname);
    memcpy(cachedBitmap, srcBitmap, rlBitmapSize * sizeof(int));
 ...
 *     USER/PROJECT). Bit i=1 means limit i matches the given value in this
 *     consumer dimension. Uses rlBitmapCache for caching with key format
 *     "value@pos". Unlike getRLBitmap(), this variant reports whether
 *     the returned bitmap is newly allocated (cache miss) or owned by the
 *     cache (cache hit), so the caller can decide whether to free it.
 *
 ...
 *     isFree [out]: Set to 1 if the returned bitmap was newly allocated on a
 *                   cache miss (caller must free it and should add it to the
 *                   cache if reuse is desired); set to 0 on a cache hit
 *                   (bitmap is owned by rlBitmapCache, do NOT free).
 *
 * Return:
 ...
 * getRLBitmap4Job
 * Description:
 *     Generate a multi-dimensional bitmap array that records whether a job
 *     matches each configured limit's consumer configuration. The bitmap has
 ...
 *     hostname [in]: Host name to match against HOST consumer
 *
 * Return:
 *     int **: Array of RL_CONSUMER_POSITION_MAX bitmap pointers, each
 *             allocated with rlBitmapSize integers. Caller must free with
 *             it. Returns NULL on error or if no limits configured.
 ********************************************************************************/
static int **
getRLBitmap4Job(struct jData *jp, char *hostname)
{
    static char fname[] = "getRLBitmap4Job";
    int **rlBitmap;
    int j;
    const char *jobValues[RL_CONSUMER_POSITION_MAX];

    if (jp == NULL || generalRLConf.nLimits <= 0) {
        return NULL;
    }
    ...
    rlBitmap = (int **)my_calloc(RL_CONSUMER_POSITION_MAX, sizeof(int *), fname);

    jobValues[RL_CONSUMER_POSITION_HOST] = hostname;
    jobValues[RL_CONSUMER_POSITION_QUEUE] =
        (jp->qPtr != NULL) ? jp->qPtr->queue : NULL;
    jobValues[RL_CONSUMER_POSITION_USER] = jp->userName;
    jobValues[RL_CONSUMER_POSITION_PROJECT] = JOB_PROJECT_NAME(jp);

    for (j = 0; j < RL_CONSUMER_POSITION_MAX; j++) {
        rlBitmap[j] = getRLBitmap(jobValues[j], j);
    }
...
static char * getLimitKey4Job(RL_LIMIT_T *limit, struct jData *jp, char *hostname) {
    ...
    if (limit == NULL || jp == NULL) {
        return NULL;
    }

    key = getLimitKey(limit, hostname,
                      (jp->qPtr != NULL) ? jp->qPtr->queue : NULL,
 ...
 *
 * Return:
 *     char *: Allocated key string. Caller must free with FREEUP().
...
getLimitKey(RL_LIMIT_T *limit, char *hostname, char *queue, char *user, char *project)
{
    ...
    /* Calculate total key length: name + # + host + # + queue + # + user + # + project + \0 */
    keyLen = strlen(limit->name) + 1; /* name# */
    for (i = 0; i < RL_CONSUMER_POSITION_MAX; i++) {
        keyLen += strlen(consumerValues[i]) + 1; /* value# */
    }

    key = (char *)my_calloc(keyLen, sizeof(char), fname);
 ...
 *
 * Input:
 *     rlBitmap [in/out]: Pointer to a multi-dimensional bitmap variable from
 *                        getRLBitmap4Job(). Three usage modes:
 *                          rlBitmap == NULL  : function allocates the bitmap
 *                                              internally and frees it on exit
 *                                              (caller never sees it)
 *                          *rlBitmap == NULL : function allocates the bitmap
 *                                              and writes it back via *rlBitmap;
 *                                              caller owns and must free it
 *                          *rlBitmap != NULL : caller-provided bitmap is used
 *                                              directly (not freed here)
 *     jp [in]: Job data pointer
 ...
 ********************************************************************************/
RL_ALLOC_RLIMIT_T *
checkRsrcLimit(int ***rlBitmap, struct jData *jp, struct hData *hp,
               LIST_T **matchedRLimits)
{
    ...
    int **bitmap = NULL;      /* the bitmap actually used for matching */
    int **localBitmap = NULL; /* bitmap allocated internally */
    int ownBitmap = FALSE;    /* TRUE if we allocated and must free on exit */
     ...
     *   rlBitmap == NULL   : allocate internally, free on exit
     *   *rlBitmap == NULL  : allocate internally, return to caller via *rlBitmap
     *   *rlBitmap != NULL  : use caller-provided bitmap directly
     */
    if (rlBitmap == NULL) {
        localBitmap = getRLBitmap4Job(jp, hp ? hp->host : NULL);
        if (localBitmap == NULL) {
            return NULL;
        }
        bitmap = localBitmap;
        ownBitmap = TRUE;
    } else if (*rlBitmap == NULL) {
        localBitmap = getRLBitmap4Job(jp, hp ? hp->host : NULL);
        if (localBitmap == NULL) {
            return NULL;
        }
        *rlBitmap = localBitmap;   /* caller owns it now */
        bitmap = localBitmap;
    } else {
        bitmap = *rlBitmap;
    }
    ...
    *matchedRLimits = listCreate("matchedRLimits");
    ...
    /* Step 1: AND all consumers bitmap together */
    andBitmap = (int *)my_calloc(rlBitmapSize, sizeof(int), fname);
    ...
    for (i = 0; i < generalRLConf.nLimits; i++) {
        ...
        key = getLimitKey4Job(limit, jp, NULL);
        if (key != NULL) {
            acctEnt = h_getEnt_(&rlAccountTab, key);
        }
        if (acctEnt != NULL) {
...
getJobRLimitOnHost(struct jData *jp, struct hData *hp, int *andBitmap)
{
    ...
    for (i = 0; i < generalRLConf.nLimits; i++) {
        ...
        key = getLimitKey4Job(limit, jp, hp->host);
        if (key != NULL) {
            acctEnt = h_getEnt_(&rlAccountTab, key);
        }
        if (acctEnt != NULL) {
 ...
 *
 * Algorithm:
 *     1. Call getRLBitmap4HLimit(host) to generate the host-dimension 
...
ckHostSlots4RLimits(struct hData *hp, struct jData *jp,
                    ...
                    RL_RESOURCE_AVAIL_T **failedRsrc)
{
    static char fname[] = "ckHostSlots4RLimits";
    int *andBitmap = NULL;
    int i, j;
    int *hostBitmap = NULL;
    ...
    *failedRsrc = NULL;
     ...
     * jobRLimitsCache.rlBitmaps[RL_CONSUMER_POSITION_HOST] */
    hostBitmap = getRLBitmap4HLimit(hp->host);
    if (hostBitmap == NULL) {
        return -1;
    }

    if (jobRLimitsCache.rlBitmaps == NULL) {
 ...
 *     3. Call getRLBitmap4HLimit(host) to generate the host-dimension bitmap and
...
getHostSlots4RLimit(struct jData *jp, struct hData *hp,
                    ...
                    RL_RESOURCE_AVAIL_T **failedRsrc)
{
    static char fname[] = "getHostSlots4RLimit";
    ...
    *failedRsrc = NULL;
    ...
    tmpCache.rlBitmaps = jobRLimitsCache.rlBitmaps;
    ...
    for (nLists = 0; nLists < 2; nLists++) {
        ...
        for (ent = listIteratorGetCurEntry(&iter);
            ...
            if (usage != NULL) {
                ...
                key = getLimitKey4Job(limitConf, jp, hostVal);
                if (key == NULL) {
                    continue;
                }
                ...
                for (j = 0; j < limitConf->nResources; j++) {
                    ...
                    usage->rsrcAccounts[j].limit = getRsrcLimitValue(limitConf, rsrc, jp->hPtr[0]);
                ...
                /* Add to rlAccountTab */
                acctEnt = h_addEnt_(&rlAccountTab, key, &newEnt);
                if (acctEnt != NULL) {
 ...
 *        a. Update host-dimension bitmap via getRLBitmap4HLimit()
...
collectRunJobRLimits4Acct(struct jData *jp, RL_JOB_LIMITS_CACHE_T *outCache)
{
    ...
    int *andBitmap = NULL;
    int i, j;
    int *hostBitmapSave = NULL; /* save host-dim bitmap to restore after
                                 * per-host refinement */
     ...
     * host consumers). We will refine per-host below. */
    bitmap = getRLBitmap4Job(jp, NULL);
    if (bitmap == NULL) {
        return;
    }

    /* Save the host-dimension row so we can restore it after per-host
     * refinement (getRLBitmap returns a cache-owned bitmap, we only
     * borrow the pointer). */
    hostBitmapSave = bitmap[RL_CONSUMER_POSITION_HOST];
     ...
     * positions), so we deduplicate adjacent entries to count unique hosts. */
    {
        ...
        for (i = 0; i < generalRLConf.nLimits; i++) {
            ...
            /* accumSlots = job's total slot usage (hPtr is slot-expanded). */
            curAllocLimit->accumSlots = jp->numHostPtr;
...

    {
        ...
        while (hIdx < jp->numHostPtr) {
            ...
            /* AND all dimensions for this host */
            hostAndBitmap = (int *)my_calloc(rlBitmapSize, sizeof(int), fname);
            for (j = 0; j < rlBitmapSize; j++) {
                hostAndBitmap[j] = bitmap[0][j];
            }
            for (i = 1; i < RL_CONSUMER_POSITION_MAX; i++) {
                for (j = 0; j < rlBitmapSize; j++) {
                    hostAndBitmap[j] &= bitmap[i][j];
                }
            }
            ...
            oneHostRLimits = getJobRLimitOnHost(jp, hp, hostAndBitmap);
            FREEUP(hostAndBitmap);
    ...
    /* Restore the host-dimension row so getRLBitmap4Job's cache is not corrupted
     * (bitmap rows are cache-owned; we only borrowed the pointers). */
    bitmap[RL_CONSUMER_POSITION_HOST] = hostBitmapSave;

    /* outCache->rlBitmaps is not used by the caller; leave as NULL */
    return;
    ...
    for (nLists = 0; nLists < 2; nLists++) {
        ...
        for (ent = listIteratorGetCurEntry(&iter);
            ...
            if (usage != NULL) {
                ...
                int newEnt;
                char *hostVal = NULL;
                struct hData *hp = (jp->hPtr != NULL && jp->numHostPtr > 0)
                    ? jp->hPtr[0] : NULL;
 ...
 *       per-host mode are converted to absolute using the first execution
 *       host's capacities.
 *
 * Algorithm:
        ...
        for (ent = listIteratorGetCurEntry(&iter);
            ...
            for (j = 0; j < limitConf->nResources; j++) {
                ...
                if (hasUsage) {
                    ...
                    float orgConsume = calcJobRsrcConsume(limitConf, rsrc,
                                                          cacheLimit,
                                                          orgResVal);
                    ...
                    struct hData *hp = (orgJob->hPtr != NULL
                                        && orgJob->numHostPtr > 0)
                        ? orgJob->hPtr[0] : NULL;
                    usedLimit = newConsume;
                    effectiveLimit = getRsrcLimitValue(limitConf, rsrc, hp);
                }
...
checkRLimits4Switch(struct jData *job, struct qData *qtp)
{
    ...
    for (nLists = 0; nLists < 2; nLists++) {
        ...
        for (ent = listIteratorGetCurEntry(&iter);
            ...
            /* Advance iterator BEFORE any potential listRemoveEntry below.
             * listIteratorNext() reads iter->curEnt->forw to advance; if we
             * remove+reinsert the current entry first, ent->forw gets rewired
             * into diffList and the iterator would follow into the wrong list.
             * Also keeps iter->curEnt in sync so listIteratorIsEndOfList()
             * correctly detects end-of-list. */
            listIteratorNext(&iter, &nextEnt);
            ...
            if (oldList != NULL && !LIST_IS_EMPTY(oldList)) {
                ...
                for (oldEnt = listIteratorGetCurEntry(&oldIter);
                    ...
                    /* For host-scoped limits, also require the hostRef to
                     * match (PER mode: same hostname; SHARED mode: same
                     * numRef semantics), so that a limit already accounted
                     * for a given host is not re-checked. */
                    if (oldLimit->limitNo == newLimit->limitNo
                        && oldLimit->hostRef.numRef == newLimit->hostRef.numRef) {
                        foundInOld = TRUE;
                        break;
                    }
                }
                       ...
                       int configOnly)
{
    ...
    for (j = 0; j < limit->nResources; j++) {
        ...
        if (!r->isPercent
            ...
            ent->resources[j].value = memValueFromMB(r->value);
        } else {
    ...
    for (j = 0; j < usage->rsrcCnt; j++) {
       ...
       /*  MEM/SWP/TMP values are stored internally in MB;
        ...
        if (ent->resources[j].type == RL_RESOURCE_TYPE_MEM
            ...
            ent->resources[j].value = memValueFromMB(ra->limit);
            ent->resources[j].used = memValueFromMB(ra->used);
        } else {
                    ...
                    int *cap, int *k)
{
    ...
    if (depth == nPer) {
        ...
        hashEnt = h_getEnt_(&rlAccountTab, key);
        FREEUP(key);
        if (hashEnt == NULL) {
